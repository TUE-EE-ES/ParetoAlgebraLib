 ...
 */
  ...
  StructureRelation escape_from_domain(const StructureRelation& r, const Domain& d, Domain& d_new)
{
	...
	if (d.is_infinite() || r.get_domains().is_some_infinite())
	{
		...
		{
			doms[i] = doms[i] * 2;
			doms[i].RealVal = r.get_domains()[i].RealVal;
	...
	for (unsigned int i = 0;i < new_doms.size();++i)
	{
		...
		new_doms[i].RealVal = r.get_domains()[i].RealVal;
	}
					   ...
					   const StructureRelation& compose_rel) const
{
	...
	StructureRelation escaped_compose_rel ((Domains) dom_range * (Domains) dom_im, compose_rel);
...
/// Equantity
/**
 * Checks equantity of two relations. This may involve renaming of one of the relations
 *
