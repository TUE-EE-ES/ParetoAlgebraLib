...
BddEquivalenceRelation::quotient(const BddSet& s) const
{
	...
	std::queue<Bdd> found_sets = adapted.get_bdd().with_geq_var(*(new_im.begin()));
...
//		hash_set<Bdd>::iterator i = found_sets.begin();
		while (!found_sets.empty())
		//while (i != found_sets.end())
		{
			const Bdd& p = found_sets.front();
			if (!((p & bdd_s).is_false()))
			res.push_back(BddSet(dom_found_sets, p & bdd_s));
