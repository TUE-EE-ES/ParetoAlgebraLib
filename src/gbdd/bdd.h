 ...
 */
	...
	// Bdd bdd_Xor(const Bdd& p1, const Bdd& p2);

	static Bdd bdd_Xor(const Bdd& p1, const Bdd& p2)
	{
		p1.space->lock_gc();

		Bdd res(p1.space, p1.space->bdd_Xor(p1.space_bdd, p2.space_bdd));
	...
	static Bdd bdd_Xnor(const Bdd& p1, const Bdd& p2)
	{
		p1.space->lock_gc();

		Bdd res(p1.space, p1.space->bdd_Xnor(p1.space_bdd, p2.space_bdd));
...
private:
	...
	void with_geq_var(Var v, std::queue<Bdd>& res) const;
...
public:
	...
	std::queue<Bdd> with_geq_var(Var v) const;
	Bdd with_image_geq_var(Bdd im, Var v) const;
	vector<Bdd> nodes() const;
...
/*
Bdd Bdd::bdd_Xor(const Bdd& p1, const Bdd& p2)
{
	p1.space->lock_gc();
