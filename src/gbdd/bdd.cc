...
/// Construct BDD representing equantity between variables
/**
...
void Bdd::with_geq_var(Bdd::Var v, std::queue<Bdd>& res) const
{
	if (bdd_is_leaf())
	{
		res.push(*this);
...
std::queue<Bdd> Bdd::with_geq_var(Bdd::Var v) const
{
	std::queue<Bdd> res;
...
int Bdd::size() const
{
	...
	int res = space->bdd_size(space_bdd);
...
vector<Bdd> Bdd::nodes() const
{
	...
	while (!explore.empty())
	{
		...
		if (!p.bdd_is_leaf())
		{
			...
			if (!ltest) { explore.push(p_then); res.push_back(p_then);}
			if (!rtest) { explore.push(p_else); res.push_back(p_else); }
		}
...
while (!explore.empty())
	{
		...
		else
		{
			...
			if (!p_then.is_false()) explore.push(p_then);
			if (!p_else.is_false()) explore.push(p_else);
		}
		explore.pop();
	}
			return n;
...
while (!explore.empty())
	{
		...
		else
		{
			...
			if (!p_then.is_false()) { explore.push(p_then); os << p.space_bdd << " -> " << p_then.space_bdd << " [style = solid];" << std::endl;}
			if (!p_else.is_false()) { explore.push(p_else); os << p.space_bdd << " -> " << p_else.space_bdd << " [style = dashed];" << std::endl;}
		}
...
/*size_t _Hash<gbdd::Bdd>::operator()(gbdd::Bdd p) const
{
	return p.space_bdd;
}*/
